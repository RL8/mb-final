...
class GreetingCrew:
    ...
    def process_name(
        self, name: str, conversation_state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Process the user's name and return the response."""
        try:
            # Update conversation state
            conversation_state.update({"has_name": True, "name": name.strip()})
    ...
    def _process_crew_output(
        self, result: str, conversation_state: Dict[str, Any]
    ) -> Dict[str, Any]:
        """Process the crew's output and extract UI components."""
        response = {"response": result, "conversation_state": conversation_state}
            ...
            if "{" in result and "}" in result:
                ...
                if isinstance(data, dict):
                    if "type" in data and "data" in data:
                        ...
                        response["response"] = (
                            result[:start] + result[end + 1 :].strip()
                        )
